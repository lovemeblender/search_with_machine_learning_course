...
def create_query(user_query, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None, useSyns=False):
    name_field = 'name'
    if use_synonyms:
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", use_syns=False):
    #### W3: classify the query
    #### W3: create filters and boosts
    # Note: you may also want to modify the `create_query` method above
    query_obj = create_query(user_query, click_prior_query=None, filters=None, sort=sort, sortDir=sortDir, source=["name", "shortDescription"], use_syns=use_syns)
...
if __name__ == "__main__":
    ...
    general.add_argument('--use_synonyms',
                         help='Are we using synonyms? The default is False')
    ...
    for line in fileinput.input():
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, use_syns=use_synonyms)
